...
#ifdef __3DS__
#include <3ds.h>
void udpError(const char* error)
{
	errorConf msg;
...
udpError("Out of memory");
	...
	/* Open the socket */
	sock->channel = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	if ( sock->channel == INVALID_SOCKET ) 
	{
		SDLNet_SetError("Couldn't create socket");
udpError("Couldn't create socket");
		goto error_return;
	}
...
udpError("Couldn't bind to local port");
//		goto error_return;
	}
...
udpError("Couldn't get socket address");
		goto error_return;
	}
